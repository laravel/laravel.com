...
class BlockQuoteRenderer implements NodeRendererInterface
{
    ...
    public function render(Node $node, ChildNodeRendererInterface $childRenderer)
    {
        ...
        if (preg_match('/^\[!(NOTE|WARNING)\](?:<br \/>)?/', $htmlPartial, $matches) === 1) {
        ...
        } elseif (preg_match('/^<strong>(Note|Warning):?<\/strong>:?(?:<br \/>)?/', $htmlPartial, $matches) === 1) {
        ...
        } elseif (preg_match('/^\{(note|tip|video)\}/', $htmlPartial, $matches) === 1) {
        ...
        $bodyPartial = str_replace($matches[0], '', $htmlPartial);
        ...
        return <<<HTML
            <div class="mb-10 max-w-2xl mx-auto px-4 py-8 shadow-lg lg:flex lg:items-center callout">
                ...
                <p class="mb-0 lg:ml-6 callout">{$bodyPartial}
            </div>
            HTML;
    }
}
